...
const user = await getKeypairFromFile('mydevwallet.json');

await airdropIfRequired(connection, user.publicKey, 1 * LAMPORTS_PER_SOL, 0.1 * LAMPORTS_PER_SOL);
...
console.log(
  `Created collection 📦! Address is: ${getExplorerLink('address', createdCollectionNft.mint.publicKey, 'devnet')}`,
);
